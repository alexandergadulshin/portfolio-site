...
const DashboardShots = ({ shots, caption }: { shots: { src: string; width: number; height: number }[]; caption: string }) => {
            ...
            {shots.map((shot, i) => (
                <figure className="site-browser" key={shot.src}>
                    ...
                    <div className="site-browser-body" style={{ aspectRatio: `${shot.width} / ${shot.height}` }}>
                        <Image src={shot.src} alt={`Screenshot of the product research dashboard ${i + 1}`} width={shot.width} height={shot.height} loading="lazy" sizes="(max-width: 991px) 92vw, 640px" />
                    </div>
                </figure>
            ))}
