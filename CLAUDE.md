...
- Images: the template ships gray placeholder images only. Every visual
  ...
  photo of the user.
- One exception to the flat-tile rule: a real screenshot the user
  captured and approved on 2026-09-11, blurred by request (client name,
  dollar figures, thumbnails): `public/assets/img/site/dashboard/radar-daily.jpg`
  on `/work/product-radar`, listed in `src/data/product-radar-shots.ts` and
  rendered by `src/components/site/DashboardShots.tsx`. The Syrex block
  (`src/data/syrex-shots.ts`) is empty on purpose: a case study may only
  show its own product's interface, and no Syrex dashboard capture exists.
  Add a screenshot only when the user names the file and it is redacted.
